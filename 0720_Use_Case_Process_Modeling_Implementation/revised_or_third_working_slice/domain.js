(function exposeApplicationDomain(root) {
  ...
  function countPending(applications) {
    return getPendingApplications(applications).length;
  }

  function getPendingApplications(applications) {
    return applications.filter(item => item.status === '審核中');
  }

  function getReviewedApplications(applications) {
    return applications.filter(item => item.status === '通過' || item.status === '退回');
  }
