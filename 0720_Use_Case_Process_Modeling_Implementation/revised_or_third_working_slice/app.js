...
function createApplicationRow(item, actionLabel) {
    ...
    reviewButton.textContent = actionLabel;
...
function renderApplications() {
  const pendingApplications = ApplicationDomain.getPendingApplications(applications);
  const reviewedApplications = ApplicationDomain.getReviewedApplications(applications);

  document.querySelector('#pendingBody').replaceChildren(
    ...pendingApplications.map(item => createApplicationRow(item, '開始審核'))
  );
  document.querySelector('#reviewedBody').replaceChildren(
    ...reviewedApplications.map(item => createApplicationRow(item, '查看結果'))
  );

  document.querySelector('#pendingCount').textContent = `${pendingApplications.length} 筆待審`;
  document.querySelector('#reviewedCount').textContent = `${reviewedApplications.length} 筆已審`;
  document.querySelector('#emptyState').hidden = pendingApplications.length > 0;
  document.querySelector('#reviewedEmptyState').hidden = reviewedApplications.length > 0;
...
function selectApplication(id) {
  ...
  if (application.status === '審核中') {
    ...
    document.querySelector('#reviewTitle').focus({ preventScroll: true });
  }
}

function fillReviewPanel(application) {
  const isPending = application.status === '審核中';
  document.querySelector('#reviewMode').textContent = isPending ? '原型審核區' : '唯讀結果回查';
  document.querySelector('#reviewTitle').textContent = isPending ? '審核申請' : '查看審核結果';
  ...
  comment.disabled = !isPending;
  document.querySelector('#approveButton').disabled = !isPending;
  document.querySelector('#returnButton').disabled = !isPending;
